...
def plot_logit(concatenated, title, colors_dict={'PRE': 'royalblue', 'FAD': 'darkorange', 'POST': 'forestgreen'},
    ...
    datapoints_plot_treatment_alignment = [0.1, 0, -0.1]
...
def plot_average_logit(df, title, colors_dict={'PRE': 'royalblue', 'FAD': 'darkorange', 'POST': 'forestgreen'}):
    ...
    for idx, treatment in enumerate(treatments):
        ...
        #     ax.plot(x_plot, curve, color=colors[treatment], alpha=0.2)
...
if __name__ == "__main__":
    ...
    directories = [path + r'\SRCF_Cannulation surgery logit data']

    subj_dict = {}
    for project_folder in directories:
        file_name_pdf = project_folder + '_surgery_logit_regression'
        ...
        #         concatenated, file_names = load_data(folder, treatment_list=('PRESURGERY', 'POSTSURGERY'))
        ...
        #                        colors_dict={'PRESURGERY': 'steelblue', 'POSTSURGERY': 'darkmagenta'})
        #
        #         pdf.savefig()
        #         plt.close(f)
